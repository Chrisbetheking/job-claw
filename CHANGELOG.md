...
- 新增统一异常分类器，区分安全验证、登录失效、身份错位、发送不确定、频率限制、页面连接、筛选失败、网络波动、AI服务、企业Provider和单岗位失效
- 普通异常不再直接触发全局熔断；系统会自动重试、刷新页面、重新连接、指数退避、降级服务或跳过当前岗位
- 新增自动恢复计划、最大尝试次数、下一次恢复时间、恢复结果和异常历史
- 连续未知异常达到阈值后先自动修复页面；普通搜索或网络问题多次修复失败后自动跳过当前组合继续，持续限频则自动延长冷却
- BOSS验证码、登录失效、HR身份不一致和发送结果不确定继续保持强制人工确认
- 修复写入招呼语阶段失败被误判为“发送结果不确定”的问题，只有实际进入发送或验证阶段才按未知发送处理

### 用户体验与每日目标

- 首页新增异常处理卡，直接展示原因、处理建议、自动恢复状态和“已处理并继续”入口
- 设置页新增异常自动恢复开关、最大尝试次数和恢复基础等待时间
- 新增今日目标进度卡，展示已完成、剩余目标、自动修复、自动跳过和需人工处理次数
- 新用户默认每日目标调整为150；既有用户设置保持不变
- OpenClaw同步数据和每日日报包含自动修复、自动跳过、需人工处理和每日目标剩余量

### 验证

- 新增异常分类与恢复单元测试
- 新增v2.2自动恢复回归测试
- 更新启动失败、失败任务重试和旧版本兼容测试

## 2.1.0 — 2026-08-01

### AI 路由与招呼语

- 海投分析默认改为 AI 优先，每个岗位优先调用已配置模型生成分析和招呼语
- 默认推荐并迁移到 `deepseek-v4-flash`，保留 OpenAI 兼容接口配置能力
- 新增本地模型路由，默认支持 Ollama `qwen3:1.7b`，低配置设备可使用 `qwen3:0.6b`
- 新增自动、仅云端、仅本地、仅轻量算法四种 AI 路由方式
- 未配置模型时在任务开始前弹出明确提醒，用户仍可选择本地轻量算法继续
- 云端或本地模型超时、断线、输出异常时，单岗位降级到本地算法，不阻塞整条海投队列

### 即时暂停

- 暂停和停止会取消正在进行的 AI Fetch 请求
- 新增 `PAUSE_NOW` 与 `STOP_NOW` 页面控制消息，打断等待、限速、筛选和发送前步骤
- 暂停按钮立即更新侧栏状态并防止重复点击，无需连续点击多次
- 保留任务位置和队列，恢复后从当前进度继续

### 本地 AI 与文档

- 新增 `install-local-ai-macos.command`，帮助安装和拉取可选 Qwen3 本地模型
- 新增本地 AI 指南、隐私说明和三层降级说明
- Release 包同时包含 OpenClaw 与本地 AI 安装脚本

## 2.0.1 — 2026-08-01

### 临时修复

- 将默认招呼语恢复为完整求职介绍 包含简历中真实存在的姓名 学校 专业 学历 到岗时间 技术 项目和负责模块
- 修复招呼语被压缩成短模板 导致表达生硬 信息不足的问题
- 修复招聘者姓名 活跃状态 薪资和学历标签混入岗位名称或招呼语的问题
- 阻止把实习生 工程师等职位名称误识别为项目经历
- 对尚未发送且内容过短的旧招呼语执行一次安全迁移 已发送历史消息保持不变
- 保持完全海投 安全海投 五项筛选 限速 去重 熔断和发送证据逻辑不变
