...
import type { JobPosting, MatchDimension, MatchResult, ResumeProfile } from './types.js';
import { clamp, includesLoose, uniqueStrings } from './utils.js';

function parseRequiredExperienceYears(text: string): number {
...
function extractJobSkills(job: JobPosting, profile: ResumeProfile): string[] {
...
export function matchJob(job: JobPosting, profile: ResumeProfile): MatchResult {
  const fullText = `${job.title}\n${job.company}\n${job.location}\n${job.description}\n${job.tags.join(' ')}`;
  const blockReasons: string[] = [];

  for (const keyword of profile.excludedKeywords) {
    if (includesLoose(fullText, keyword)) blockReasons.push(`命中排除关键词：${keyword}`);
  }
