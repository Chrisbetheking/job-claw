...
import type { ClaimCheck, GreetingResult, JobPosting, MatchResult, ResumeProfile } from './types.js';
import { includesLoose, normalizeText, uniqueStrings } from './utils.js';

function projectEvidence(profile: ResumeProfile, job: JobPosting): { name: string; fact: string } | null {
  const text = `${job.title} ${job.description} ${job.tags.join(' ')}`;
  let best: { name: string; fact: string; score: number } | null = null;

  for (const project of profile.projects) {
    const score = project.keywords.filter((keyword) => includesLoose(text, keyword)).length;
    const fact = project.facts[0];
    if (fact && (!best || score > best.score)) best = { name: project.name, fact, score };
  }

  return best ? { name: best.name, fact: best.fact } : null;
}

export function validateGreeting(text: string, profile: ResumeProfile): GreetingResult {
...
export function generateGreeting(job: JobPosting, profile: ResumeProfile, match: MatchResult): GreetingResult {
