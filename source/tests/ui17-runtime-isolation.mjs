...
if (document.documentElement.dataset.jobclawContentVersion !== '2.0.1') throw new Error('DOM runtime stamp missing');
const probe = await new Promise(resolve => listeners[0]({ type: 'PROBE' }, {}, resolve));
if (!probe.ok || probe.contentVersion !== '2.0.1' || probe.contentFile !== 'content-v37.js') throw new Error('versioned probe failed');
if (!content.includes("const JOBCLAW_CONTENT_VERSION = '2.0.1'")) throw new Error('content version constant missing');
