...
if (document.documentElement.dataset.jobclawContentVersion !== '1.7.0') throw new Error('DOM runtime stamp missing');
const probe = await new Promise(resolve => listeners[0]({ type: 'PROBE' }, {}, resolve));
if (!probe.ok || probe.contentVersion !== '1.7.0' || probe.contentFile !== 'content-v37.js') throw new Error('versioned probe failed');
if (!content.includes("const JOBCLAW_CONTENT_VERSION = '1.7.0'")) throw new Error('content version constant missing');
