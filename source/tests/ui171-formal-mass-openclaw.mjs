...
const [manifest, html, background, common, safety, bridge, nativeHost, installer] = await Promise.all([
  ...
  readFile('../install-openclaw-macos.command', 'utf8')
]);

if (!manifest.permissions.includes('nativeMessaging')) throw new Error('nativeMessaging permission missing');
for (const token of ['value="full-mass"', 'value="safe-mass"', 'id="massApplyAnalysis"', 'id="pacingPreset"', 'id="dailyReportEnabled"', 'id="dailyReportTime"', 'id="generateBridgeReport"']) {
