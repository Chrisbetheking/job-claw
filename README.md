...
![Version](https://img.shields.io/badge/version-v1.7.0-078A83)
...
| 企业风险核验 | 通过 OpenClaw Provider 接入授权企业数据源，未配置时使用本地岗位风险规则提示 |
| 安全投递策略 | 支持精准投递和安全海投，提供保守、标准、高效及自定义节奏 |
| 去重与限额 | 对岗位指纹、历史投递和同一公司每日数量进行限制 |
| 模拟运行 | 完成采集、核验、分析和队列预览，但不执行真实发送 |
| 执行模式 | 支持人工确认，也支持由用户主动启用的自动辅助模式 |
| 沟通草稿 | 根据简历证据和岗位要求生成可编辑的应聘沟通内容 |
| 任务进度 | 每个任务和岗位均可显示独立进度、阶段、结果和异常原因 |
| 失败恢复 | 异常任务可重新打开、单条重试、标记忽略或由用户继续处理 |
| 更新提醒 | 定期检查 GitHub Release，发现新版本后在侧边栏和系统通知中提示 |
| OpenClaw | HTTP与Native Messaging双通道，用于OCR、企业Provider、自动唤醒、每日汇报和本地恢复 |


## v1.7 正式版

v1.7将岗位筛选和执行安全拆开处理。岗位策略可以宽松，但真实发送仍然保持单线程、去重、限速、身份绑定、发送证据和异常熔断。

- **精准投递**：按照用户设置的匹配阈值筛选，适合技术岗和方向明确的岗位
- **安全海投**：只阻止明确硬性冲突、重复岗位和高风险岗位，技能、专业、学历与年限差距只影响排序
- **海投快速模式**：不逐岗位请求LLM，使用简历真实事实生成基础招呼语，减少等待和模型成本
- **AI个性化海投**：需要更深分析时再为每个岗位调用AI
- **投递效率**：提供保守、标准、高效和自定义档位，遇到页面变慢或网络异常时自动降速

OpenClaw增加HTTP与Native Messaging双通道，安装完成后可自动唤醒本地桥接。每天可在指定时间生成投递日报并通过macOS通知中心汇报。连接和日报说明见 [OpenClaw连接与每日汇报](docs/OPENCLAW_CONNECTION_AND_REPORT.md)。
...
普通的岗位分析、沟通草稿生成和人工确认不强制安装 OpenClaw。安装后可使用企业Provider、本地OCR、自动唤醒和每日求职汇报。
...
推荐先阅读 [GitHub 工作流](docs/GITHUB_WORKFLOW.md)。
