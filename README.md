...
[安装](#安装) · [AI 配置](#ai-配置) · [功能](#功能) · [自动恢复](docs/auto-recovery.md) · [开发](#本地开发) · [贡献](CONTRIBUTING.md) · [许可证](#许可证)

![Version](https://img.shields.io/badge/version-v2.2.0-078A83)
...
v2.2 将原来的“连续失败就全局熔断”改为 **异常分级诊断与自动恢复**。普通页面超时、网络波动、筛选未生效、聊天输入区未加载、AI 或企业 Provider 暂时不可用时，系统会自动重试、降速、刷新页面、降级或跳过当前岗位，并从原进度继续。
...
侧栏会显示异常原因、自动处理状态和具体建议。默认每日投递目标调整为 150，并展示剩余目标、自动修复、自动跳过和需要人工处理的次数。目标是持续执行，而不是为了凑数量绕过平台安全机制或重复发送。
...
- **异常自动恢复**：分级诊断、页面自修复、网络退避、岗位跳过、模型降级和断点续跑
...
1. 在仓库 **Releases** 下载 `JobClaw-v2.2.0.zip`
2. 解压 ZIP
...
```text
单岗位异常 → 自动重试一次或冷却该岗位 → 继续后续队列
页面/筛选异常 → 自动重新连接或刷新页面 → 从断点继续
网络/频率异常 → 自动退避和降速 → 冷却后继续
验证码/登录/身份错位/发送不确定 → 给出明确建议并等待用户确认
```

设置页可以配置自动恢复开关、最大尝试次数和基础等待时间。多次修复仍失败时，普通搜索组合会自动跳过并继续下一个；持续限频会自动延长冷却，不要求用户反复重置。关闭自动恢复后，普通异常仍会记录建议，但不会自动刷新页面。
...
完整测试包含 Manifest V3、CSP、语法、敏感信息扫描、AI 路由、即时暂停、异常自动恢复和历史发送回归。
